...
def render_hidden_settings_panel(settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with st.expander("Einstellungen", expanded=is_visible):
        if not is_visible:
            st.info("Der Einstellungsbereich ist vorhanden, aber aktuell verborgen.")
            st.caption(f"Tastenkombination zum Einblenden: {settings.get('shortcut_combo', 'ctrl+alt+s')}")
            if st.button("Einstellungen einblenden", key="settings_show_button", use_container_width=True):
