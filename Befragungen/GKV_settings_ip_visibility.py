...
def render_bottom_settings_expander(settings: Dict[str, Any]) -> Tuple[Optional[Any], Dict[str, Any]]:
    ...
    if not settings_visible_for_ip(settings, current_ip):
    ...
    with st.expander("Einstellungen", expanded=False):
        ...
        with tabs[4]:
            ...
            st.text_input(
                "Test-/Override-IP",
                value=str(st.session_state.get("manual_client_ip", "") or ""),
                key="manual_client_ip",
                help="Optional für Tests. Wenn gesetzt, wird diese IP für die Sichtbarkeitsprüfung verwendet.",
            )
