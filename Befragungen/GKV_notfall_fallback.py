...
def settings_visible_for_file(settings: Dict[str, Any]) -> bool:
    if not bool(settings.get("settings_file_visibility_enabled", False)):
        return True

    required_name = str(settings.get("settings_required_filename", "name.xlsx") or "").strip()
    if not required_name:
        return False

    target = get_script_dir() / required_name
    return target.exists() and target.is_file()


def normalize_machine_name(value: str) -> str:
    return str(value or "").strip().lower()


def parse_machine_lines(text_value: str) -> List[str]:
    return [
        normalize_machine_name(x)
        for x in re.split(r"[\n,;]+", str(text_value or ""))
        if normalize_machine_name(x)
    ]


def get_current_machine_name() -> str:
    manual_name = normalize_machine_name(st.session_state.get("manual_machine_name", ""))
    if manual_name:
        return manual_name

    candidates = []
    for env_key in ["COMPUTERNAME", "HOSTNAME"]:
        env_val = normalize_machine_name(os.environ.get(env_key, ""))
        if env_val:
            candidates.append(env_val)

    try:
        host = normalize_machine_name(socket.gethostname())
        if host:
...
def settings_visible_for_machine(settings: Dict[str, Any], current_machine_name: str) -> bool:
    if not bool(settings.get("settings_machine_protection_enabled", False)):
        return True

    allowed_names = [
        normalize_machine_name(name)
        for name in settings.get("settings_allowed_machine_names", [])
        if normalize_machine_name(name)
    ]
    if not allowed_names:
        return False

    return normalize_machine_name(current_machine_name) in allowed_names
...
def settings_visible_for_ip(settings: Dict[str, Any], current_ip: str) -> bool:
    ...
    raw_allowed = settings.get("settings_allowed_ips", [])
    if isinstance(raw_allowed, str):
        raw_allowed = parse_ip_lines(raw_allowed)

    allowed_rules = [
        rule for rule in raw_allowed
        if is_valid_ip_rule(str(rule))
    ]

    # Notfall-Fallback:
    # Wenn IP-Schutz aktiv ist, aber keine gültigen Regeln hinterlegt sind,
    # bleibt der Expander sichtbar statt sich komplett auszusperren.
...
def render_bottom_settings_expander(settings: Dict[str, Any]) -> Tuple[Optional[Any], Dict[str, Any]]:
    ...
    machine_is_allowed = settings_visible_for_machine(settings, current_machine_name)
    file_is_allowed = settings_visible_for_file(settings)

    if not machine_is_allowed or not file_is_allowed:
        return uploaded_file, settings

    with st.expander("Einstellungen", expanded=False):
        st.caption("Der Bereich ist unten im UI platziert. Die Sichtbarkeit kann optional über das Vorhandensein einer Datei im Skriptordner gesteuert werden.")
        ...
        with tabs[4]:
            draft["settings_file_visibility_enabled"] = st.checkbox(
                "Expander nur sichtbar, wenn Datei existiert",
                value=bool(settings.get("settings_file_visibility_enabled", False)),
                key="cfg_settings_file_visibility_enabled",
            )
            draft["settings_required_filename"] = st.text_input(
                "Erforderlicher Dateiname",
                value=str(settings.get("settings_required_filename", "name.xlsx")),
                key="cfg_settings_required_filename",
                help="Datei im selben Ordner wie dieses Skript. Beispiel: name.xlsx",
            )

            required_name = str(draft.get("settings_required_filename", "name.xlsx") or "").strip()
            required_path = get_script_dir() / required_name if required_name else get_script_dir()
            exists_now = required_path.exists() and required_path.is_file() if required_name else False

            st.caption(f"Skriptordner: {get_script_dir()}")
            st.caption(f"Geprüfte Datei: {required_path}")
            st.caption(f"Status: {'vorhanden' if exists_now else 'nicht vorhanden'}")
        ...
        with tabs[5]:
            ...
            draft["shortcut_combo"] = st.text_input(
                ...
                help="Nur als Einstellung gespeichert. Der Bereich selbst ist dauerhaft sichtbar.",
            )

        with tabs[6]:
            st.caption(f"Einstellungsdatei: {SETTINGS_FILE}")
            st.write("Dieser Expander bleibt unten im UI sichtbar und kann direkt geöffnet werden.")
