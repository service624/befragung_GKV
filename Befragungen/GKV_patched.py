...
def render_input(question: Question) -> None:
    ...
    if "ganze zahl" in input_kind:
        value = st.number_input("Antwort eingeben", step=1, format="%d", key=num_key)
    ...
    if input_kind in {"zahl", "eur", "%", "prozent"} or "zahl" in input_kind or "eur" in input_kind or "%" in input_kind:
        ...
        value = st.text_input(label, key=text_key, placeholder="z. B. 1234,56")
        if st.button("Weiter", key=f"continue_{question.number}"):
...
st.write("Excel-Datei hochladen, Fragenpfad automatisch durchlaufen und Antworten am Ende als CSV oder Excel exportieren.")
