...
RUNTIME_DIR = Path(__file__).with_name("gkv_runtime")
RUNTIME_DIR.mkdir(parents=True, exist_ok=True)


def save_uploaded_workbook(uploaded_file: Any) -> Path:
    suffix = Path(str(getattr(uploaded_file, "name", "") or "GKV.xlsx")).suffix or ".xlsx"
    target = RUNTIME_DIR / f"active_workbook{suffix}"
    with open(target, "wb") as f:
        f.write(uploaded_file.getbuffer())
    return target


def get_writeable_cell(ws, row_idx: int, col_idx: int):
...
def find_row_for_question(ws, question_no: str) -> Optional[int]:
    target = normalize_question_no(question_no)
    if not target:
        return None

    for row_idx in range(1, ws.max_row + 1):
        a_val = normalize_question_no(ws.cell(row=row_idx, column=1).value)
        target_cell = get_writeable_cell(ws, row_idx, 3)
        if a_val == target and target_cell is not None:
            return row_idx

    for row_idx in range(1, ws.max_row + 1):
        b_val = normalize_question_no(ws.cell(row=row_idx, column=2).value)
        target_cell = get_writeable_cell(ws, row_idx, 3)
        if b_val == target and target_cell is not None:
            return row_idx

    return None


def write_answers_to_excel_result_workbook(workbook_path: Any, answers: Dict[str, Any]) -> None:
    workbook_path = Path(str(workbook_path))
    wb = load_workbook(workbook_path)
    try:
        ws_input = wb["Tabelle1"] if "Tabelle1" in wb.sheetnames else wb[wb.sheetnames[0]]
        for question_no, answer_value in (answers or {}).items():
            row_idx = find_row_for_question(ws_input, question_no)
            if not row_idx:
                continue

            target_cell = get_writeable_cell(ws_input, row_idx, 3)
            if target_cell is None:
                continue

            if isinstance(answer_value, list):
                value_to_write = ", ".join([str(x) for x in answer_value if str(x).strip()])
            else:
                value_to_write = answer_value
...
def recalculate_workbook_with_soffice(workbook_path: Any) -> Path:
    workbook_path = Path(str(workbook_path))
    out_dir = RUNTIME_DIR / "recalc_out"
    ...
    recalc_path = out_dir / workbook_path.name
    if recalc_path.exists():
        return recalc_path

    raise RuntimeError((result.stderr or result.stdout or "LibreOffice-Recalc fehlgeschlagen.").strip())


def read_excel_results(workbook_path: Any) -> Dict[str, Any]:
    workbook_path = Path(str(workbook_path))
    try:
        workbook_to_read = recalculate_workbook_with_soffice(workbook_path)
    except Exception:
        workbook_to_read = workbook_path

    wb = load_workbook(workbook_to_read, data_only=True)
    try:
        ws_result = wb["Tabelle4"] if "Tabelle4" in wb.sheetnames else wb[wb.sheetnames[-1]]
        return {
            "A1": ws_result["A1"].value,
            "A2": ws_result["A2"].value,
            "A3": ws_result["A3"].value,
            "B1": ws_result["B1"].value,
            "B2": ws_result["B2"].value,
            "B3": ws_result["B3"].value,
        }
    finally:
        wb.close()


def calculate_results_via_excel(workbook_path: Any, answers: Dict[str, Any]) -> Dict[str, Any]:
    write_answers_to_excel_result_workbook(workbook_path, answers)
    return read_excel_results(workbook_path)
...
def render_settings_expander(settings: Dict[str, Any]) -> Tuple[Optional[Any], Dict[str, Any]]:
    ...
    # Fragen sollen auch dann weiter funktionieren, wenn der Expander unsichtbar ist.
    # Deshalb wird eine bereits gespeicherte Upload-Datei trotzdem geladen.
    saved_path = get_saved_upload_path(settings)
    if saved_path:
        uploaded_source = str(saved_path)

    if not settings_expander_visible():
...
def reset_questionnaire(keep_questions: bool = False) -> None:
    ...
    workbook_path = st.session_state.get("workbook_path") if keep_questions else None
...
def render_result_page() -> None:
    ...
    if workbook_path:
        if st.session_state.get("excel_result_values") is None:
            try:
                st.session_state.excel_result_values = calculate_results_via_excel(
                    workbook_path,
                    st.session_state.answers,
                )
                st.session_state.excel_result_error = ""
            except Exception as exc:
                st.session_state.excel_result_error = str(exc)

        st.markdown("### Excel-Ergebnis")
        if st.session_state.get("excel_result_error"):
            st.error(f"Excel-Ergebnis konnte nicht gelesen werden: {st.session_state.excel_result_error}")
        elif st.session_state.get("excel_result_values"):
            result_values = st.session_state.excel_result_values
            cols = st.columns(3)
            cols[0].metric(str(result_values.get("A1") or "Ergebnis 1"), str(result_values.get("B1") or ""))
            cols[1].metric(str(result_values.get("A2") or "Ergebnis 2"), str(result_values.get("B2") or ""))
            cols[2].metric(str(result_values.get("A3") or "Ergebnis 3"), str(result_values.get("B3") or ""))
    ...
    if not history_df.empty:
        ...
        with c2:
            st.download_button(
                ...
                data=xlsx_bytes,
                file_name="antwortverlauf.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
            )

    if st.button("Fragebogen neu starten", use_container_width=True):
        reset_questionnaire(keep_questions=True)
        st.session_state.excel_result_values = None
        st.session_state.excel_result_error = ""
        st.rerun()
...
if uploaded_file is not None:
    ...
    if hasattr(uploaded_file, "getbuffer"):
        ...
        workbook_path = Path(str(uploaded_file))

    reload_needed = (
        st.session_state.uploaded_filename != current_filename
        or st.session_state.questions is None
        or str(st.session_state.get("workbook_path") or "") != str(workbook_path)
    )
    if reload_needed:
        try:
            questions, sheet_name = load_questions_from_excel(workbook_path)
            st.session_state.questions = questions
            st.session_state.uploaded_filename = current_filename
            st.session_state.sheet_name = sheet_name
            st.session_state.workbook_path = str(workbook_path)
            st.session_state.excel_result_values = None
            st.session_state.excel_result_error = ""
            reset_questionnaire(keep_questions=True)
        except Exception as exc:
